...
function bugFieldsText(bug: any): string {
  return [
    ...
    bug.suggested_response ? `${bug.mode === 'full_auto' ? "gameLM's actual response/action" : 'gameLM suggested response'}: ${bug.suggested_response}` : null,
    bug.mode === 'full_auto' && bug.resolution_outcome ? `Resolution outcome (Full Auto engagement signal): ${bug.resolution_outcome}` : null,
