...
// Gold-standard few-shot examples (sourced from verified BetSaracen tickets, window 5/30–6/9).
// 3 CORRECTION · 4 ENHANCEMENT · 4 PREFERENCE
// NONE examples omitted — eval-issue is never called when there is no edit.
...
async function evalRow(client: Anthropic, row: IssueRow): Promise<EvalResult> {
  ...
  const msg = await client.messages.create({
    ...
    max_tokens: 256,
    system:     EVAL_SYSTEM,
    messages:   [...fewShotMessages, { role: 'user', content: buildUserMessage(row) }],
  })
