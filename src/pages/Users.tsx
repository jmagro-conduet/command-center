...
export default function Users() {
  ...
  async function handleRoleChange(u: DBUser, newRole: 'admin' | 'agent' | 'qa') {
            ...
            {filtered.map((u, i) => (
              <tr key={u.authId} style={{ borderBottom: i < filtered.length - 1 ? '1px solid rgba(0,0,0,0.06)' : 'none' }}>
                ...
                <td style={{ padding: '14px 16px' }}>
                  <select
                    value={u.role}
                    onChange={e => handleRoleChange(u, e.target.value as 'admin' | 'agent' | 'qa')}
                    style={{
                      ...
                      color: u.role === 'admin' ? '#6b21a8' : u.role === 'qa' ? '#0369a1' : '#58595B',
                    }}
                  >
